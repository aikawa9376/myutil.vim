...
call util#set_default(
      \ 'g:util#directory_mru_path',
      \ s:substitute_path_separator(s:base.'/directory'))
call util#set_default(
      \ 'g:util#directory_mru_limit',
      \ 1000, 'g:unite_source_directory_mru_limit')
call util#set_default(
      \ 'g:util#directory_mru_ignore_pattern',
      \'\%(^\|/\)\.\%(hg\|git\|bzr\|svn\)\%($\|/\)'.
      \'\|^\%(\\\\\|/mnt/\|/media/\|/temp/\|/tmp/\|\%(/private\)\=/var/folders/\)',
      \ 'g:unite_source_directory_mru_ignore_pattern')
call util#set_default('g:util#follow_links', 0)
...
let s:mru = {
      ...
      \ 'type'            : '',
      \ 'mtime'           : 0,
      \ 'update_interval' : g:util#update_interval,
      \ 'mru_file'        : '',
      \ 'limit'           : {},
      \ 'do_validate'     : g:util#do_validate,
      \ 'is_loaded'       : 0,
      \ 'version'         : s:VERSION,
      \ }
...
function! s:mru.gather_candidates(args, context) abort
  ...
  if a:context.is_redraw && g:util#do_validate
    call self.reload()
  endif
...
function! s:resolve(fpath) abort
  return g:util#follow_links ? resolve(a:fpath) : a:fpath
endfunction
...
let s:file_mru = extend(deepcopy(s:mru), {
      \ 'type'          : 'file',
      \ 'mru_file'      : s:expand(g:util#file_mru_path),
      \ 'limit'         : g:util#file_mru_limit,
      \ }
      \)
...
let s:directory_mru = extend(deepcopy(s:mru), {
      \ 'type'          : 'directory',
      \ 'mru_file'      : s:expand(g:util#directory_mru_path),
      \ 'limit'         : g:util#directory_mru_limit,
      \ }
      \)
...
function! util#init() abort
endfunction
function! util#_import_file(path) abort
  let path = a:path
  if path == ''
...
function! util#_import_directory(path) abort
  let path = a:path
  if path == ''
...
function! util#_get_mrus() abort
  return s:MRUs
endfunction
function! util#_append() abort
  if &l:buftype =~ 'help\|nofile' || &l:previewwindow
    return
  endif
  call util#append(s:expand('%:p'))
endfunction
function! util#_gather_file_candidates() abort
  return util#_get_mrus().file.gather_candidates([], {'is_redraw': 0})
endfunction
function! util#_gather_directory_candidates() abort
  return util#_get_mrus().directory.gather_candidates([], {'is_redraw': 0})
endfunction

function! util#append(filename) abort
...
function! util#_reload() abort
  for m in values(s:MRUs)
    call m.reload()
  endfor
endfunction
function! util#_save(...) abort
  let opts = a:0 >= 1 && type(a:1) == type({}) ? a:1 : {}
...
function! util#_abbr(path, time) abort
  let abbr = (g:util#time_format == '') ? '' :
          \ strftime('(' . g:util#time_format . ') ', a:time)
  let abbr .= a:path
  return abbr
endfunction
...
function! s:is_file_exist(path) abort
  let ignore = !empty(g:util#file_mru_ignore_pattern)
        \ && a:path =~ g:util#file_mru_ignore_pattern
  return !ignore && (getftype(a:path) ==# 'file' ||
			  \ getftype(a:path) ==# 'link' ||
			  \ a:path =~ '^\h\w\+:')
endfunction
function! s:is_directory_exist(path) abort
  let ignore = !empty(g:util#directory_mru_ignore_pattern)
        \ && a:path =~ g:util#directory_mru_ignore_pattern
...
function! s:print_error(msg) abort
  echohl Error | echomsg '[util] ' . a:msg | echohl None
endfunction
